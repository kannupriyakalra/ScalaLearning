...
/*
1. Perimeter of a irregular polygon = sum of each side length = Side1 + Side2 + Side3 + ... + SideN

2. side length = distance between 2 points when their coordinates are given (x1, y1) and (x2, y2) = √ { (x2-x1)^2 + (y2-y1)^2 } //search "
how to calculate distance between 2 points when their coordinates are given" in chatgpt

3. To pair adjacent elements of a sequence in Scala and create a list of those pairs, you can use the sliding method. //search "
how pair adjacent elements of a sequence in scala and make a list of them " in chatgpt
...
object ComputeThePerimeterOfAPolygon {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val points: Seq[(Int, Int)] = for (nItr <- 1 to n) yield {
    ...
    val edges: List[Seq[(Int, Int)]] = points.sliding(2).toList // edges: List[Seq[(Int, Int)]] every Seq here has 2 elements ie List(Seq(p1, p2), Seq(p1, p2), ... , Seq(p1, p2))
    val perimeterOfAPolygon = edges.map {
      case Seq(p1, p2) => distanceBetween2Points(p1, p2)
