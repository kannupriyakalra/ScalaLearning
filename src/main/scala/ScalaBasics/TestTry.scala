...
object TestTry extends App{
  ...
  def divide: Try[Int] = {
    ...
    val problem: Try[Int] = dividend.flatMap(x => divisor.map(y => x/y))  //as you have to divide 2 try[Int] so do it via flatmap n map to extract value of x n y, divisor.map(10/0) --returns failure
    // 2nd approach is via pattern match divdend match 2 cases success n failure n in success divisor match 2 cases success n failure
    problem match {
      case Success(v) =>
      ...
      case Failure(e) =>
        ...
        divide //if you put in wrong i/p then call divide again so you are given a second chance incase you entered wrong i/p.
    }
  }
