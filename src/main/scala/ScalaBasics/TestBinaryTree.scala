...
-Implementation of map in a binary tree recursively.
 */
object TestBinaryTree extends App {
...
Leaf has no data, its empty. When map is called on Leaf it returns Leaf or empty same thing as when we call f function on empty then we get empty output. If a tree is leaf,
there is no meaning of map, as tree is empty, so we have to return a Tree[B], we return Empty as its a object of type Tree[Nothing] and as per covariance , Tree[A], Tree[B], Tree[Int],
Tree[String], Any are all supertypes of Tree[Nothing], Empty is a object of Tree[Nothing], value inside Tree[A] ie A is covariant, so when map converts it to make
...
value: A, left: Tree[A], right: Tree[A] , we have to return Tree[B], Tree[B] has 2 variations Leaf or Node, can't return Leaf as logically wrong, so we return Node.
apply f on value as value is of type A,
